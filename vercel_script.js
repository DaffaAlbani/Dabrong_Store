...
const FALLBACK_PACKAGES = [
  { product_id:'MCGG5',  product_name:'5 Diamonds (5 + 0 Bonus)',    price:1402,  diamond:5,  bonus:'', cat:'starter' },
  { product_id:'MCGG12', product_name:'12 Diamonds (11 + 1 Bonus)',   price:3400,  diamond:12, bonus:'', cat:'starter' },
  { product_id:'MCGG19', product_name:'19 Diamonds (17 + 2 Bonus)',   price:5400,  diamond:19, bonus:'', cat:'starter' },
  { product_id:'MCGG28', product_name:'28 Diamonds (25 + 3 Bonus)',   price:7900,  diamond:28, bonus:'', cat:'starter' },
  { product_id:'MCGG44', product_name:'44 Diamonds (40 + 4 Bonus)',   price:11900, diamond:44, bonus:'', cat:'starter' },
];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const GAMES_CONFIG = {
    mobilelegends: {
      ...
      theme: { accent: '#8b5cf6', accentL: '#a78bfa', accentD: '#6d28d9', glow: 'rgba(139, 92, 246, 0.4)', gradient: 'linear-gradient(135deg, #6d28d9, #3b82f6)' }
    },
    freefire: {
      category: 'AGFF',
      title: 'Free Fire',
  ...
  function updateCheckoutSummary() {
    ...
    if (summaryItem) {
      if (s.selectedPkg) {
        ...
        const icon = isPass ? '🎫' : (config.itemIcon || '💎');
        const unit = config.unitName || 'Diamond';
        summaryItem.textContent = isPass ? s.selectedPkg.product_name : `${icon} ${(s.selectedPkg.diamond||0).toLocaleString('id-ID')} ${unit}`;
      } else {
  ...
  function renderPkgs() {
    ...
    pkgGrid.innerHTML = filtered.map(pkg => {
      ...
      const gemIcon = isPass ? '🎫' : (config.itemIcon || '💎');
      const unit = config.unitName || 'Diamond';
      const displayAmt = isPass ? rawName : `${pkg.diamond.toLocaleString('id-ID')} ${unit}`;
  ...
  window.selectPkg = function(id) {
    ...
    if (s.selectedPkg) {
      ...
      const icon = isPass ? '🎫' : (config.itemIcon || '💎');
      const unit = config.unitName || 'Diamond';
      const displayInfo = isPass ? s.selectedPkg.product_name : `${icon} ${(s.selectedPkg.diamond||0).toLocaleString('id-ID')} ${unit}${s.selectedPkg.bonus ? ' ' + s.selectedPkg.bonus : ''}`;
  ...
  function checkStep2Next() {
    if (btnS2Nxt) btnS2Nxt.disabled = false;
  }

  const inpWaEl = document.getElementById('inp-wa');
  if (inpWaEl) {
    inpWaEl.addEventListener('input', checkStep2Next);
  }
  ...
  btnS2Nxt.addEventListener('click', async () => {
    // 1. Auto-fill or validate User ID & Server ID if not set via verification button
    const uidInp = document.getElementById('inp-uid');
    const srvInp = document.getElementById('inp-srv');
    const config = GAMES_CONFIG[s.selectedGame];

    let uid = s.playerId || (uidInp ? uidInp.value.trim() : '');
    let srv = s.serverId || (srvInp ? srvInp.value.trim() : '');

    if (!uid) {
      showToast('Masukkan User ID player kamu terlebih dahulu', 'alert-triangle');
      if (uidInp) uidInp.focus();
      return;
    }

    if (config && config.hasServer && !srv) {
      showToast('Masukkan Server ID player kamu terlebih dahulu', 'alert-triangle');
      if (srvInp) srvInp.focus();
      return;
    }
    ...
    // 2. Validate package selection
    if (!s.selectedPkg) {
      showToast('Pilih nominal top-up dahulu', 'alert-triangle');
      return;
    }

    // 3. Validate WhatsApp number
    const waRaw = document.getElementById('inp-wa')?.value.trim() || '';
    let cleanWa = waRaw.replace(/[\s\-\+]/g, '');
    if (cleanWa.startsWith('628')) {
      cleanWa = '08' + cleanWa.slice(3);
    }

    if (!cleanWa || !/^(08|628)\d{7,12}$/.test(cleanWa)) {
      showToast('Nomor WhatsApp tidak valid (contoh: 08123456789 atau 628123456789)', 'alert-triangle');
      document.getElementById('inp-wa')?.focus();
      return;
    }
    ...
    try {
      ...
      btnS2Nxt.textContent = 'Beli Sekarang';
    }
  });
  ...
  function fillPaymentInstructions(order, waUrl) {
    ...
    if (order.payment_method === 'qris') {
      ...
      noteText.innerHTML = `⚠️ Pindai QRIS di atas (nominal <strong>Rp ${order.total_bayar.toLocaleString('id-ID')}</strong> terisi otomatis saat di-scan).`;
      
      try {
        const dynamicQRIS = generateDynamicQRIS(order.total_bayar);
        document.getElementById('pi-qris-img').src = `https://api.qrserver.com/v1/create-qr-code/?size=350x350&data=${encodeURIComponent(dynamicQRIS)}`;
      } catch (e) {
    ...
    // Midtrans Snap Pay handler
    const midtransContainer = document.getElementById('midtrans-pay-container');
    const btnMidtrans = document.getElementById('btn-pay-midtrans');
    if (midtransContainer && btnMidtrans) {
      midtransContainer.classList.remove('hidden');
      btnMidtrans.onclick = async () => {
        btnMidtrans.disabled = true;
        btnMidtrans.textContent = '⏳ Memuat Midtrans...';
        try {
          ...
          if (data.success && data.snap_token) {
            if (window.snap) {
              window.snap.pay(data.snap_token, {
                onSuccess: function(result) {
                  showToast('Pembayaran berhasil! Memproses item...', 'check-circle');
                  setTimeout(() => window.location.href = `/status?order=${order.order_no}`, 2000);
                },
                onPending: function(result) {
                  showToast('Menunggu pembayaran...', 'clock');
                },
                onError: function(result) {
                  showToast('Pembayaran gagal atau dibatalkan', 'x-circle');
                },
                onClose: function() {
            ...
            showToast(data.message || 'Gagal memuat pembayaran Midtrans', 'x-circle');
          }
        } catch (e) {
          showToast('Gagal terhubung ke Midtrans', 'x-circle');
        } finally {
          btnMidtrans.disabled = false;
          btnMidtrans.textContent = '⚡ Bayar Otomatis via Midtrans (QRIS & Bank)';
        }
      };
    }
...
/* ==========================================
   EMVCo DYNAMIC QRIS GENERATOR (Dabrong Store)
   ========================================== */
function generateDynamicQRIS(amount) {
  const staticQRIS = "00020101021126570011ID.DANA.WWW011893600915303309664402090330966440303UMI51440014ID.CO.QRIS.WWW0215ID10265451123550303UMI5204654053033605802ID5913Dabrong Store6015Kota Probolingg61056723763046FF0";
